...
\begin{itemize}
  \item \textbf{State space} - \texttt{gym.spaces.Box(low=-2, high=8, shape=HxW)}: The state space of the custom MDP 
...
\begin{itemize}
  ...
  \textit{stack-based}. The coords of the cells of the cells with no mine around are pushed
...
\begin{frame}{MDP modelling: action masking + reward normalization}
  \twocol{
    \textbf{Action masking}
      ...
      To let the agent \textit{risk} more, Q-values
      (for off-policy algorithms) or logits (for on policy algorithm)
