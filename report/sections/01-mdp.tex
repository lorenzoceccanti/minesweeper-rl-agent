...
\begin{frame}{MDP modelling: the state space}
  ...
  \begin{itemize}
    ...
    \item $-1$: this is \textbf{not actually a possible state} for the agent. It's only the way in which
          a mine is represented in the board containing the \textit{solution} of the game.\\
          The agent is informed about losing a game by the \texttt{terminated} boolean variable of the \textit{Gym Env}.\\
          The agent is \underline{never informed about the location of the mines}.
...
\begin{frame}{MDP modelling: the reward (first version)}
  ...
  \begin{enumerate}
    ...
    \item During \textit{a progression} more than one cell could
          be opened. However, the reward was still $+1$ \textbf{independently} on the number of the \textbf{opened cells}.
    \item The overall reward was dependent on the board size.
  \end{enumerate}
\end{frame}
...
\begin{frame}{MDP modelling: action masking + reward normalization}
  ...
  \[
    ...
    \begin{cases}
      ...
      -\dfrac{0.5}{S}
       & \text{already revealed cell is selected}
      \\[0.18cm]
